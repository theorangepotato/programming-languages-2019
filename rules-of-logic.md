...
[Last lecture](https://hackmd.io/s/S1KcSWeYQ) we looked in some detail at a proof stating that if a program is finitely branching then termination can always be shown by a measure function to the natural numbers. 
...
- To learn the [rules of logic](https://github.com/alexhkurz/programming-languages/blob/master/Gentzen-natural-deduction-rules.png).

- To explain the idea that all of formal, deductive reasoning can be reduced to [this small set of rules](https://github.com/alexhkurz/programming-languages/blob/master/Gentzen-natural-deduction-rules.png). 

- Going back to our [lecture on induction](), we then understand that [this small set of rules](https://github.com/alexhkurz/programming-languages/blob/master/Gentzen-natural-deduction-rules.png) is an inductive definition of the set of all provable statements in (as far as we know) all of mathematics.[^principia]
...
>>>>>> Define $\; x=a$. We need to check two statements.
...
>>>>>>>> Then $a\stackrel{+}{\longrightarrow}a$.
>>>>>>>> Which contradicts that $(A,\to)$ is terminating.

>>>>>>> We showed not $b\stackrel{\ast}{\longrightarrow}a$.
...
>We showed that if $A$ is finitely branching and terminating, then there is a measure function $\phi:A\to\mathbb N$.
...
**Exercise**: Find examples of applications of these rules in the proof of the theorem of the [previous lecture](https://hackmd.io/s/S1KcSWeYQ). 

**Exercise**: Which of the other rules of [Gentzen](https://github.com/alexhkurz/programming-languages/blob/master/Gentzen-natural-deduction-rules.png) can you identify in the proof of the [previous lecture](https://hackmd.io/s/S1KcSWeYQ)?

**Exercise:** Think about the following: Is it possible to reduce all steps in [this proof](https://hackmd.io/s/S1KcSWeYQ) to an application of one of [this rules](https://github.com/alexhkurz/programming-languages/blob/master/Gentzen-natural-deduction-rules.png)? What about other proofs or even all proofs?

**Exercise**: Give a formal proof, using [Gentzen rules](https://github.com/alexhkurz/programming-languages/blob/master/Gentzen-natural-deduction-rules.png), that every number divisible by 4 is divisible by 2. You may use the laws of arithmetic freely. [Hint: $x$ is divisible by $n$ can be formalised as $\exists y\,.\, x=n\cdot y$.]
